...
import { Send, CheckCircle, XCircle, Mail, Phone, MapPin, LucideIcon } from 'lucide-react';
...
// ── Data ───────────────────────────────────────────────────────────────────
const contactInfo: { Icon: LucideIcon; label: string; value: string; href: string | null }[] = [
  { Icon: Mail,   label: 'Email',    value: 'chandakanish0018@gmail.com', href: 'mailto:chandakanish0018@gmail.com' },
  { Icon: Phone,  label: 'Phone',    value: '+91 74358 86257',            href: 'tel:+917435886257' },
  { Icon: MapPin, label: 'Location', value: 'Silvassa, India · Remote OK', href: null },
];
...
const FloatingField: React.FC<FloatingFieldProps> = ({
  label, name, type = 'text', value, onChange, multiline = false, required = false, valid,
}) => {
  ...
  return (
    <div className="relative">
      <Tag
        ...
        required={required} rows={multiline ? 4 : undefined} placeholder=""
        className={cn(
      ...
      <label
        ...
        className={cn(
          ...
          floating ? 'top-2 text-xs text-electric-400' : 'top-4 text-sm text-white/30',
        )}
      >
        {label}
      </label>
...
const InfoCard: React.FC<InfoCardProps> = ({ Icon, label, value, href, index }) => {
  ...
  const inner = (
    <div className="flex items-start gap-4 relative z-[1]">
      ...
      <div>
        ...
        <p className="text-sm text-white/65 group-hover:text-white/90 transition-colors">{value}</p>
      </div>
    </div>
  );
  ...
  return (
    ...
    >
      ...
      <motion.div className="absolute inset-0 rounded-2xl pointer-events-none" style={{ background: spotlight }} />
      {href
  ...
  return (
    ...
    >
      ...
      <div className="relative z-[3] max-w-6xl mx-auto px-6 sm:px-8">
        ...
        >
          <span className="font-mono text-xs text-electric-400 tracking-widest uppercase">05 / Contact</span>
          ...
          >
            {REMARKABLE.map((char, i) => (
              <motion.span
                ...
                transition={{ delay: i * 0.04, duration: 0.4, ease: [0.33, 1, 0.68, 1] }}
              >
                {char}
              </motion.span>
            ))}
        ...
        >
          Have a project in mind or want to explore collaboration? I'd love to hear from you.
        </motion.p>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="flex flex-col gap-4 lg:col-span-1">
            {contactInfo.map(({ Icon, label, value, href }, i) => (
              <InfoCard key={label} Icon={Icon} label={label} value={value} href={href} index={i} />
            ))}
          </div>
          ...
          >
            <form onSubmit={handleSubmit} className="flex flex-col gap-4">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <FloatingField label="Your name"      name="name"    value={form.name}    onChange={handleChange} required valid={form.name.length > 1} />
                <FloatingField label="Email address"  name="email"   type="email" value={form.email}   onChange={handleChange} required valid={isValidEmail} />
              </div>
              <FloatingField   label="Subject"         name="subject" value={form.subject} onChange={handleChange} required valid={form.subject.length > 3} />
              <FloatingField   label="Message"         name="message" value={form.message} onChange={handleChange} multiline required valid={form.message.length > 10} />
              ...
              <div className="pt-2">
                <motion.button
                  ...
                  className={cn(
                    ...
                    status === 'success' ? 'bg-neon-green/20 border border-neon-green/30 text-neon-green'
